...
void SD_DETECT_Init(void)
{
	...
	HAL_GPIO_Init(SD_DETECT_PIN_Port, &GPIO_InitStruct);
}

//SD插拔状态检测
uint8_t SD_IsDetected(void)
{
	__IO uint8_t status = SD_OUT;
  
	//读取SD卡检测引脚
	if(HAL_GPIO_ReadPin(SD_DETECT_PIN_Port, SD_DETECT_Pin) == GPIO_PIN_RESET)
	{
		status = SD_IN;
	}
	...
	{ 
		uint8_t SD_root[]="0:";
		while(SD_Init())		//检测到SD卡后初始化SD卡
		{
			printf("SD Card Init Error!\r\n");
			HAL_Delay(500);
			PIN_SET(RUN_LED_PIN_Port, RUN_LED_Pin); 
			HAL_Delay(500);
			PIN_RESET(RUN_LED_PIN_Port, RUN_LED_Pin); 
		}
		//为fatfs文件系统申请内存
		exfuns_init();
		//挂载SD卡
		f_mount(fs[0],"0:",1);
		//遍历SD卡内的文件
		mf_scan_files(SD_root);
	}  
	else
	{
		printf("Not detected SD Card\r\n");
	}
	return status;
}

//SPI硬件层初始化
//设置硬件上与SD卡相关联的控制引脚输出(片选引脚)
void SD_SPI_Init(void)
{
	...
	//PA4
    GPIO_Initure.Pin=SDSS_Pin; 
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;          	//上拉
    GPIO_Initure.Speed=GPIO_SPEED_FREQ_HIGH;    //高速
    HAL_GPIO_Init(GPIOA,&GPIO_Initure);
	
	HAL_GPIO_WritePin(GPIOA,SDSS_Pin,GPIO_PIN_SET);
